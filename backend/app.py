...
TEMP_IMAGE_FOLDER = os.path.join(BASE_DIR, "backend", "temp_images")
...
def save_face_image():
    try:
        ...
        # Save the image
        with open(image_path, "wb", buffering=0) as img_file:
...
def record_attendance(student_id):
    try:
        payload = {
            ...
            "CheckinDate": datetime.utcnow().strftime("%Y-%m-%d"),
            "CheckinTime": datetime.utcnow().strftime("%H:%M:%S")
        }
...
FACE_ENCODINGS_PATH = os.path.join(BASE_DIR, "backend", "face_encodings.pkl")
...
def process_attendance():
    try:
        ...
        if recognized_student_id != "Unknown":
            ...
            response = requests.post(
                f"{https://ikrjvtpgoteoimpkxfvy.supabase.co}/rest/v1/attendance",
                headers=SUPABASE_HEADERS,
...
# New route to display mess bill history
@app.route('/mess_bill_history')
def mess_bill_history():
    try:
        # Query attendance records (adjust table name/columns as needed)
        response = requests.get(
            f"{SUPABASE_URL}/rest/v1/attendance?select=student_id,checkin_date,students(name)",
            headers=SUPABASE_HEADERS
        )

        if response.status_code != 200:
            logging.error(f"❌ Failed to fetch attendance records: {response.text}")
            return jsonify({"error": "Failed to fetch mess bill data"}), 500

        attendance_records = response.json()

        # Group attendance records by student and month
        mess_bill = {}
        for record in attendance_records:
            student_id = record.get("student_id")
            # 'students' is expected to be a nested object containing the student name.
            student_name = record.get("students", {}).get("name", "Unknown")
            date_str = record.get("checkin_date")  # Expected in YYYY-MM-DD format
            if not date_str:
                continue

            # Convert the date string to a datetime object and then to a month-year string
            date_obj = datetime.strptime(date_str, "%Y-%m-%d")
            month_year = date_obj.strftime("%B %Y")

            # Create a unique key based on student and month
            key = (student_id, student_name, month_year)
            # Each present day adds Rs. 145 to the bill
            mess_bill[key] = mess_bill.get(key, 0) + 145

        # Prepare a list for the template
        mess_bill_list = [
            {"student_id": key[0], "student_name": key[1], "month": key[2], "bill": bill}
            for key, bill in mess_bill.items()
        ]

        return render_template("mess_bill_history.html", mess_bill_list=mess_bill_list)

    except Exception as e:
        logging.error(f"❌ Error fetching mess bill history: {e}")
        return jsonify({"error": "Internal server error"}), 500
